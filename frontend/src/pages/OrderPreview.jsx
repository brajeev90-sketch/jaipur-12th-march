...
export default function OrderPreview() {
  ...
  const handleExportPdf = () => {
    // Generate clean HTML for PDF - 12px font, Table first, Notes flow to next page
    ...
    const itemsHtml = order.items.map((item, index) => {
      ...
      return `
        <div class="item-page">
          <!-- Header -->
          <div style="display: flex; justify-content: space-between; align-items: flex-start; padding-bottom: 6px; border-bottom: 2px solid #3d2c1e; margin-bottom: 6px;">
            ...
            <table style="border: 1px solid #3d2c1e; border-collapse: collapse; font-size: 10px;">
              <tr style="border-bottom: 1px solid #3d2c1e;">
                <td style="padding: 3px 6px; background: #f5f0eb; font-weight: bold; border-right: 1px solid #3d2c1e;">ENTRY DATE</td>
                <td style="padding: 3px 6px; min-width: 100px;">${formatDateDDMMYYYY(order.entry_date)}</td>
              </tr>
              <tr style="border-bottom: 1px solid #3d2c1e;">
                <td style="padding: 3px 6px; background: #f5f0eb; font-weight: bold; border-right: 1px solid #3d2c1e;">FACTORY</td>
                <td style="padding: 3px 6px;">${order.factory || '-'}</td>
              </tr>
              <tr style="border-bottom: 1px solid #3d2c1e;">
                <td style="padding: 3px 6px; background: #f5f0eb; font-weight: bold; border-right: 1px solid #3d2c1e;">SALES ORDER</td>
                <td style="padding: 3px 6px; font-family: monospace;">${order.sales_order_ref || '-'}</td>
              </tr>
              <tr>
                <td style="padding: 3px 6px; background: #f5f0eb; font-weight: bold; border-right: 1px solid #3d2c1e;">BUYER PO</td>
                <td style="padding: 3px 6px; font-family: monospace;">${order.buyer_po_ref || '-'}</td>
              </tr>
            </table>
          </div>
          ...
          <div style="display: flex; gap: 10px; padding: 6px 0;">
            <div style="width: 70%;">
              ${mainImage 
                ? `<img src="${mainImage}" alt="Product" style="width: 100%; height: 280px; object-fit: contain; border: 1px solid #ddd;" />`
                : `<div style="width: 100%; height: 280px; display: flex; align-items: center; justify-content: center; background: #f5f5f5; border: 1px solid #ddd; color: #888; font-size: 12px;">No Image</div>`
              }
            </div>
            <div style="width: 30%;">
              ${item.leather_image || item.leather_code ? `
                <div style="margin-bottom: 6px;">
                  ${item.leather_image 
                    ? `<img src="${item.leather_image}" alt="Leather" style="width: 100%; height: 90px; object-fit: cover; border: 1px solid #ddd;" />`
                    : `<div style="width: 100%; height: 90px; background: #8B4513;"></div>`
                  }
                  <p style="font-size: 10px; text-align: center; margin: 3px 0 0 0;">LEATHER: ${item.leather_code || '-'}</p>
                </div>
              ` : ''}
              ${item.finish_image || item.finish_code ? `
                <div>
                  ${item.finish_image 
                    ? `<img src="${item.finish_image}" alt="Finish" style="width: 100%; height: 90px; object-fit: cover; border: 1px solid #ddd;" />`
                    : `<div style="width: 100%; height: 90px; background: #d4a574;"></div>`
                  }
                  <p style="font-size: 10px; text-align: center; margin: 3px 0 0 0;">FINISH: ${item.finish_code || '-'}</p>
                </div>
              ` : ''}
            </div>
          </div>
          ...
          <table style="width: 100%; border-collapse: collapse; font-size: 12px; margin: 6px 0;">
            <thead>
              <tr style="background: #3d2c1e; color: white;">
                ...
                <th style="padding: 6px; text-align: center; border: 1px solid #3d2c1e;">H</th>
                <th style="padding: 6px; text-align: center; border: 1px solid #3d2c1e;">D</th>
                <th style="padding: 6px; text-align: center; border: 1px solid #3d2c1e;">W</th>
            ...
            <tbody>
              <tr>
                ...
                <td style="padding: 6px; border: 1px solid #3d2c1e; font-size: 11px;">${item.description || '-'}</td>
          ...
          <div class="notes-section">
            <div style="background: #3d2c1e; color: white; padding: 6px 8px; font-weight: bold; font-size: 12px;">Notes:</div>
            <div style="padding: 8px; font-size: 12px; line-height: 1.5;">${notesContent || '<span style="color:#888;">No notes</span>'}</div>
          </div>
          
          <!-- Footer -->
          <div style="display: flex; justify-content: space-between; margin-top: 6px; padding-top: 6px; border-top: 1px solid #ddd; font-size: 10px; color: #666;">
  ...
  const handlePrint = () => {
    // Generate clean HTML for printing - 12px font, Table first, Notes flow
    ...
    // Generate HTML for each item - 12px font
    const itemsHtml = order.items.map((item, index) => {
      const cbm = item.cbm_auto 
        ? ((item.height_cm || 0) * (item.depth_cm || 0) * (item.width_cm || 0) / 1000000).toFixed(2)
        : item.cbm;
